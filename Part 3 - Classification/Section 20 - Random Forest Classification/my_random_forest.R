...
classifier <- randomForest(x = train_set[-3], y = train_set[,3,drop=TRUE], ntree = 15)

# Create vector of probabilities for test set, then assuming border value of 0.5 change them to final 0/1 outcomes
prediction <- predict(classifier, newdata = test_set[-3], type="class")
...
xs_grid_prediction <- predict(classifier, newdata = xs_grid, type="class")
...
# Skip scaling and plot decision tree
dataset <- read.csv("Social_Network_Ads.csv")
dataset <- dataset[, 3:5]
set.seed(123)
dataset$Purchased = factor(dataset$Purchased, levels = c(0, 1))
