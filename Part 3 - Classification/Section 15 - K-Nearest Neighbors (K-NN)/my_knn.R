...
# Create KNN predictions for test set
y_pred <- knn(train = train_set[, 1:2], test = test_set[, 1:2], cl = train_set[, 3], k = 5)

# Check the accuracy with confusion matrix
confusion_matrix <- table(y_pred, test_set$Purchased)
...
xs_grid_prediction <- knn(train = train_set[, 1:2], test = xs_grid, cl = train_set[, 3], k = 5)
