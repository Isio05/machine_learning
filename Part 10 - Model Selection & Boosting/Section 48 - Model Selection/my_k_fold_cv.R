...
folds <- createFolds(y = train_set$Purchased, k = 10)
cross_val <- lapply(folds, function(x){
  train_fold <- train_set[-x, ]
  test_fold <- train_set[x, ]
  classifier <- svm(formula=Purchased~., data=train_fold, type = "C-classification", kernel = "radial")
  prediction <- predict(classifier, type = "response", newdata = test_fold[1:2])
  cm <- table(prediction, test_fold$Purchased)
